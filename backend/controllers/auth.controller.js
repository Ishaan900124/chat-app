...
export const signup = async (req, res) => {
	try {
		const { fullName, username, password, confirmPassword, gender, profile_pic } = req.body;
		...
		const profile = profile_pic === "" ? (gender === "male" ? boyProfilePic : girlProfilePic) : "";
		...
		const newUser = new User({
			...
			gender,
			profilePic: profile_pic? profile_pic : profile,
		});
...
export const update = async (req,res) => {
    try {
        ...
        res.status(200).json({
			...
			profilePic: userInfomation.profilePic,
		});
    } catch (error) {
